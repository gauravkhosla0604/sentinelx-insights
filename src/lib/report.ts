...
<<<<<<< HEAD
  ...
  ${list(
    buildAttackStory(email, threat, campaign).map(
      (s) => `${s.title} — ${s.explanation} (Evidence: ${s.evidence})`,
    ),
    "Insufficient evidence to reconstruct an attack path",
  )}
...
export function buildReportText(input: ReportInput): string {
  ...
  const caseId = evidence?.evidenceId || "Not available";
  const urls = email.urls.length ? email.urls.map((u) => u.url).join("\n") : "Not available";
  const indicators = threat.factors
    .filter((f) => f.triggered)
    .map((f) => `- [${f.severity.toUpperCase()}] ${f.title}: ${f.explanation} | Evidence: ${f.evidence || "Not available"}`)
    .join("\n");
  const actions = [
    `Quarantine the message matching subject "${email.subject || "Not available"}"`,
    email.domains.length ? `Block domains: ${email.domains.join(", ")}` : "Block domains: Not available",
    email.urls.length ? `Block URLs: ${email.urls.map((u) => u.url).join(", ")}` : "Block URLs: Not available",
    "Search mailboxes for related messages and preserve the original .eml with its SHA-256 hash",
  ].join("\n");

  return [
    "SENTINELX INVESTIGATION REPORT",
    `Case ID: ${caseId}`,
    `Threat level: ${threat.level}`,
    `Risk score: ${threat.score}/100`,
    `Attack type: ${campaign?.attackType || "Not available"}`,
    `Sender: ${email.from.address || "Not available"}`,
    ...
    "",
    "Attack Story:",
    attackStoryPlainText(buildAttackStory(email, threat, campaign)),
    "",
    "Evidence summary:",
    `SHA-256: ${evidence?.sha256 || "Not available"}`,
    `Analyzed at: ${evidence?.analyzedAt || "Not available"}`,
    `IPs: ${email.ips.length ? email.ips.join(", ") : "Not available"}`,
    `Domains: ${email.domains.length ? email.domains.join(", ") : "Not available"}`,
    "",
    "Recommended actions:",
    actions,
  ].join("\n");
